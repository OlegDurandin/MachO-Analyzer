...
```
### Плагин анализа подписей ``sign_analyzer_plugin.py``

Плагин предназначен для анализа подписи Mach-O файлов, включая проверку сертификатов, прав и статуса подписи.
Для анализа используется встроенная системная утилита ``codesign``.

#### Основные возможности

1. **Анализ статуса подписи**:
   - Valid - подпись валидна
   - Invalid - подпись невалидна
   - Not signed - файл не подписан
   - Error - ошибка при проверке
   - Revoked - сертификат отозван
   - Expired - сертификат истек
   - Untrusted - сертификат не доверен

2. **Типы подписей**:
   - Ad-hoc - локальная подпись
   - Developer ID - подпись разработчика
   - Apple Development Certificate - подпись Apple
   - Unknown - неизвестный тип

3. **Информация о разработчике**:
   - Authority - орган сертификации
   - Team ID - идентификатор команды разработчиков
   - Timestamp - временная метка подписи

4. **Анализ прав (Entitlements)**:
   - Security sensitive - критические для безопасности разрешения
   - Hardware access - доступ к оборудованию
   - Data access - доступ к данным
   - Network - сетевые разрешения
   - System - системные разрешения
   - Development - разрешения для разработки
   - Other - прочие разрешения

#### Пример вывода

```
Выполняется команда: codesign -dvv /Applications/Telegram.app/Contents/MacOS/Telegram
...
┡━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━┩
│ Статус          │ Valid                   │
│ Тип подписи     │ Developer ID            │
│ Временная метка │ 26 Mar 2025 at 17:27:54 │
└─────────────────┴─────────────────────────┘
...
┃ Параметр  ┃ Значение                                                      ┃
┡━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┩
│ Authority │ Developer ID Application: TELEGRAM MESSENGER LLP (6N38VWS5BX) │
│ Team ID   │ 6N38VWS5BX                                                    │
└───────────┴───────────────────────────────────────────────────────────────┘
...
Security Sensitive
┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
┃ Entitlement                                      ┃ Значение                                                                                                                                ┃
┡━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┩
│ com.apple.security.app-sandbox                   │ False                                                                                                                                   │
│ com.apple.security.application-groups            │ ['6N38VWS5BX.ru.keepcoder.Telegram', '6N38VWS5BX.ru.keepcoder.Telegram.TelegramShare', '6N38VWS5BX.ru.keepcoder.Telegram.FocusIntents'] │
...
Подпись кода в macOS - это механизм безопасности, который обеспечивает целостность и подлинность исполняемых файлов. Каждый подписанный файл содержит цифровую подпись, которая создается с использованием криптографических алгоритмов и сертификатов.

#### Компоненты системы подписи

1. **Сертификаты**:
   - Apple Root Certificate - корневой сертификат Apple
   - Developer ID Certificate - сертификат разработчика
   - Apple Development Certificate - сертификат для разработки
   - Ad-hoc Certificate - локальный сертификат

2. **Типы подписей**:
   - Developer ID - для распространения приложений через App Store и вне его
   - Apple Development - для разработки и тестирования
   - Ad-hoc - для локального использования

3. **Entitlements (Разрешения)**:
   - Определяют права и возможности приложения
   - Контролируют доступ к системным ресурсам
   - Управляют безопасностью приложения

#### Процесс подписи

1. **Создание подписи**:
   - Генерация хеша исполняемого файла
   - Шифрование хеша с помощью приватного ключа
   - Добавление сертификата и зашифрованного хеша в файл

2. **Проверка подписи**:
   - Извлечение сертификата и подписи
   - Проверка валидности сертификата
   - Верификация хеша файла
   - Проверка целостности

#### Безопасность

1. **Защита от модификации**:
   - Любое изменение файла делает подпись недействительной
   - Система блокирует выполнение неподписанных файлов
   - Предотвращает внедрение вредоносного кода

2. **Контроль доступа**:
   - Gatekeeper проверяет подписи при запуске
   - Система проверяет цепочку доверия сертификатов
   - Контролирует права доступа через entitlements

3. **Политики безопасности**:
   - Hardened Runtime - дополнительные меры защиты
   - Library Validation - проверка загружаемых библиотек
   - Debugging Restrictions - ограничения на отладку

#### Практическое применение

1. **Разработка**:
   - Использование Apple Development сертификатов
   - Настройка entitlements для доступа к API
   - Тестирование на реальных устройствах

2. **Распространение**:
   - Подпись Developer ID для App Store
   - Подпись для вне магазина
   - Управление профилями распространения
...
1. **Системные требования**:
   - macOS 10.15+ для новых требований
   - Поддержка старых версий
   - Совместимость с разными архитектурами

2. **Ограничения**:
   - Необходимость обновления сертификатов
   - Ограничения на отладку
   - Требования к хранению ключей

3. **Отладка и тестирование**:
   - Специальные разрешения для разработчиков
   - Возможности временного отключения защиты
   - Инструменты для анализа подписей



### Плагин анализа разрешений ``permission_analyzer_plugin.py``

Плагин предназначен для анализа разрешений (permissions) в Mach-O файлах, включая права доступа и entitlements.


##### Основные возможности

1. **Анализ базовых разрешений**:
   - Проверка прав доступа к файлам
   - Анализ системных разрешений
   - Проверка прав на выполнение

2. **Анализ Entitlements**:
   - Проверка прав доступа к системным ресурсам
   - Анализ сетевых разрешений
   - Проверка прав на доступ к данным


### Плагин анализа отладочных символов ``debug_analyzer_plugin.py``

Плагин предназначен для анализа отладочной информации в Mach-O файлах, включая DWARF и другие отладочные данные.

#### Теоретическая часть

1. **Типы отладочной информации**:
   - DWARF (Debug With Arbitrary Record Format) - современный формат отладочной информации
   - STABS - устаревший формат отладочной информации
   - dSYM - отдельный файл с отладочными символами

2. **Основные секции DWARF**:
...
3. **Типы символов STABS**:
   - N_GSYM (0x20) - глобальный символ
   - N_STSYM (0x26) - статический символ
   - N_SO (0x64) - имя исходного файла
   - N_SOL (0x84) - имя исходного файла (include файл)
   - N_FUN (0x24) - имя функции

4. **Символы защиты**:
   - `___stack_chk_guard` - защита от переполнения стека
   - `___stack_chk_fail` - обработчик ошибки защиты стека

#### Особенности реализации
#### Особенности реализации

1. **Анализ DWARF секций**:
   - Поиск сегмента `__DWARF`
   - Проверка наличия важных DWARF секций
   - Анализ размера секций

2. **Анализ символов**:
   - Чтение таблицы символов (LC_SYMTAB)
   - Анализ таблицы строк
   - Определение типов символов
   - Извлечение имен функций и файлов

3. **Проверка dSYM**:
   - Поиск UUID в LC_UUID
   - Проверка наличия dSYM файла в стандартных местах:
     - Рядом с исполняемым файлом
     - В директории DerivedData
     - В стандартных местах Xcode

4. **Классификация символов**:
   - Отладочные символы (__dbg_, __asan_, __ubsan_)
   - Символы отладчиков (__gdb_, __lldb_)
   - Символы Objective-C (_objc_debug)
   - Символы DWARF (_DWARF, __DWARF)
...
│ DWARF              │ Нет                              │
│ DWARF Info         │ Нет                              │
│ Отладочные символы │ Да                               │
│ STABS              │ Да                               │
│ dSYM UUID          │ C72BC84F93DE3CE5871A13F5D495EA94 │
└────────────────────┴──────────────────────────────────┘
```



### Плагин анализа обфускации ``obfuscation_analyzer_plugin.py``

Плагин предназначен для анализа Mach-O файлов на наличие признаков обфускации кода и потенциально вредоносных техник.
...
1. **Анализ энтропии**:
   - **Определение**:
     - Мера неопределенности или случайности в данных
     - В контексте реверс-инжиниринга: показатель сложности или обфускации кода
   
   - **Применение**:
     - Выявление зашифрованных или сжатых данных
     - Определение упакованных секций
     - Обнаружение обфусцированного кода
   
   - **Интерпретация результатов**:
     - Низкая энтропия (0-4): обычный код или текст
     - Средняя энтропия (4-7): сжатые данные или обфусцированный код
     - Высокая энтропия (7-8): зашифрованные данные или упакованный код
   
   - **Типичные значения**:
     - Обычный код: 4.5-5.5
     - Сжатые данные: 6.0-7.0
     - Зашифрованные данные: 7.0-8.0
     - Случайные данные: 8.0

2. **Анализ потока управления**:
   - **Косвенные вызовы** (indirect calls):
     - Использование регистров или памяти для хранения адресов функций
     - Затрудняет статический анализ, так как адрес функции вычисляется во время выполнения
     - Типичные паттерны: `call eax`, `call [eax]`, `jmp eax`
   
   - **Таблицы переходов** (jump tables):
     - Структуры данных, содержащие адреса переходов
     - Часто используются для обфускации условных переходов
     - Затрудняют понимание логики программы
     - Типичные паттерны: `jmp [eax*4 + offset]`

   - **Полиморфный код**:
     - Код, который изменяет сам себя во время выполнения
     - Использует косвенные вызовы и динамическое вычисление адресов
     - Затрудняет отладку и анализ

3. **Методы обхода анализа**:
   - **Противодействие анализу потока управления**:
     - Использование виртуальных машин
     - Динамическая генерация кода
     - Шифрование строк во время выполнения
   
   - **Противодействие анализу энтропии**:
     - Смешивание зашифрованных и обычных данных
     - Использование сжатия вместо шифрования
     - Динамическая распаковка кода

4. **Практическое применение**:
   - **Обнаружение вредоносного ПО**:
     - Выявление упакованных вирусов
     - Определение обфусцированных бэкдоров
     - Анализ полиморфных вредоносных программ
   
   - **Защита интеллектуальной собственности**:
     - Обнаружение попыток обхода защиты
     - Анализ методов реверс-инжиниринга
     - Оценка эффективности обфускации

#### Функциональные возможности

1. **Анализ энтропии**:
   - Расчет энтропии для каждой секции
   - Порог энтропии: 7.0
   - Анализ сегментов `__TEXT` и `__DATA`
...
3. **Анализ потока управления**:
   - Подсчет косвенных вызовов (0xff 0xd0, 0xff 0xe0)
   - Подсчет таблиц переходов (0xff 0x24 0xc5)
   - Оценка подозрительности потока управления

4. **Паттерны анализа**:
   - Кодирование: base64, hex, rot13, xor, aes, rc4
   - Подозрительные: cmd, shell, exec, system, eval
...
┡━━━━━━━━━╇━━━━━━━━━━╇━━━━━━━━━━━┩
│ __TEXT  │ 6.51     │ Нормально │
│ __DATA  │ 6.59     │ Нормально │
└─────────┴──────────┴───────────┘
...
┡━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┩
│ Закодированные строки   │ 8          │ hex, xor, aes, rc4, base64, hex, xor, aes                                                                                                                                                                 │
│ Подозрительные паттерны │ 286        │ /System/Library/Frameworks/SceneKit.framework/Versions/A/SceneKit, /System/Library/Frameworks/CoreMediaIO.framework/Versions/A/CoreMediaIO,                                                               │
│                         │            │ /System/Library/Frameworks/Carbon.framework/Versions/A/Carbon, /System/Library/Frameworks/WebKit.framework/Versions/A/WebKit, /System/Library/Frameworks/MetalKit.framework/Versions/A/MetalKit,          │
│                         │            │ /System/Library/Frameworks/Metal.framework/Versions/A/Metal, /System/Library/Frameworks/CoreSpotlight.framework/Versions/A/CoreSpotlight,                                                                 │
│                         │            │ /System/Library/Frameworks/Contacts.framework/Versions/A/Contacts, /System/Library/Frameworks/QuartzCore.framework/Versions/A/QuartzCore,                                                                 │
│                         │            │ /System/Library/Frameworks/Security.framework/Versions/A/Security, /System/Library/Frameworks/VideoToolbox.framework/Versions/A/VideoToolbox,                                                             │
│                         │            │ /System/Library/Frameworks/Accelerate.framework/Versions/A/Accelerate, /System/Library/Frameworks/AppKit.framework/Versions/C/AppKit,                                                                     │
│                         │            │ /System/Library/Frameworks/LocalAuthentication.framework/Versions/A/LocalAuthentication, /System/Library/Frameworks/CoreServices.framework/Versions/A/CoreServices,                                       │
│                         │            │ /System/Library/Frameworks/CoreVideo.framework/Versions/A/CoreVideo, /System/Library/Frameworks/CoreMedia.framework/Versions/A/CoreMedia, /System/Library/Frameworks/OpenGL.framework/Versions/A/OpenGL,  │
│                         │            │ /System/Library/Frameworks/IOKit.framework/Versions/A/IOKit, /System/Library/Frameworks/QuickLook.framework/Versions/A/QuickLook,                                                                         │
│                         │            │ /System/Library/Frameworks/JavaScriptCore.framework/Versions/A/JavaScriptCore, /System/Library/Frameworks/MapKit.framework/Versions/A/MapKit,                                                             │
│                         │            │ /System/Library/Frameworks/Quartz.framework/Versions/A/Quartz, /System/Library/Frameworks/Foundation.framework/Versions/C/Foundation,                                                                     │
│                         │            │ /System/Library/Frameworks/AVFoundation.framework/Versions/A/AVFoundation, /System/Library/Frameworks/MediaPlayer.framework/Versions/A/MediaPlayer, /usr/lib/libSystem.B.dylib,                           │
│                         │            │ /System/Library/Frameworks/AVKit.framework/Versions/A/AVKit, /System/Library/Frameworks/ApplicationServices.framework/Versions/A/ApplicationServices,                                                     │
│                         │            │ /System/Library/Frameworks/AudioToolbox.framework/Versions/A/AudioToolbox, /System/Library/Frameworks/CFNetwork.framework/Versions/A/CFNetwork,                                                           │
│                         │            │ /System/Library/Frameworks/CoreAudio.framework/Versions/A/CoreAudio, /System/Library/Frameworks/CoreFoundation.framework/Versions/A/CoreFoundation,                                                       │
│                         │            │ /System/Library/Frameworks/CoreGraphics.framework/Versions/A/CoreGraphics, /System/Library/Frameworks/CoreImage.framework/Versions/A/CoreImage,                                                           │
│                         │            │ /System/Library/Frameworks/CoreLocation.framework/Versions/A/CoreLocation, /System/Library/Frameworks/CoreText.framework/Versions/A/CoreText,                                                             │
│                         │            │ systemGreenColor, systemInfo, systemLangCode, systemLocale, systemOrangeColor, systemRedColor, systemVersion, threadExecutionWidth, timeFixOrSaltsMissing, updateIsActiveValue, willChangeValueForKey:,   │
│                         │            │ withUpdatedSystemCode:, _pGtAESayAFGt, G_AESDyAC                                                                                                                                                          │
│ Всего строк             │ 498877     │                                                                                                                                                                                                           │
└─────────────────────────┴────────────┴───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
...
┡━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━╇━━━━━━━━━━━━━━━┩
│ Косвенные вызовы  │ 21071    │ Подозрительно │
│ Таблицы переходов │ 1        │ Нормально     │
│ Общий статус      │          │ Подозрительно │
└───────────────────┴──────────┴───────────────┘
...
┃ Уровень               ┃ Причины                                                           ┃ Рекомендации                                   ┃
┡━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┩
│ SeverityLevel.WARNING │ Обнаружено значительное количество подозрительных паттернов (294) │ Проверить все подозрительные паттерны          │
│                       │ Обнаружен подозрительный поток управления                         │ Проверить косвенные вызовы и таблицы переходов │
└───────────────────────┴───────────────────────────────────────────────────────────────────┴────────────────────────────────────────────────┘
```

### Плагин анализа механизмов  ``persistence_analyzer.py``

Плагин предназначен для анализа механизмов персистентности в Mach-O файлах, которые могут использоваться вредоносным ПО для сохранения присутствия в системе.

##### Основные возможности

1. **Анализ механизмов автозапуска**:
   - Launch Agents
   - Login Items
   - Cron Jobs
   - System Extensions
...
2. **Анализ модификаций системы**:
   - Изменения в plist файлах
   - Модификации ассоциаций файлов
   - Изменения системных настроек

3. **Анализ подозрительных системных вызовов**:
   - chmod, chown
   - setuid, setgid
   - seteuid, setegid
   - setreuid, setregid
...
2. **Факторы оценки**:
   - Количество найденных механизмов
   - Типы механизмов персистентности
   - Количество подозрительных вызовов
   - Веса механизмов

3. **Рекомендации**:
   - Проверка механизмов автозапуска
...
┡━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━╇━━━━━━━━━━━━━━┩
│ Cron Jobs          │ 1          │ atd          │
│ Plist Modification │ 1          │ PropertyList │
└────────────────────┴────────────┴──────────────┘
...
┃ Уровень               ┃ Причины                                 ┃ Рекомендации                                    ┃
┡━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┩
│ SeverityLevel.WARNING │ Средний риск персистентности (score: 8) │ Проверить все механизмы автозапуска             │
...
Mach-O Analyzer допускает разработку сторонних плагинов, для легкости расширения функционала.
В качестве примера, разработан 

### Плагин анализа паттернов вредоносного кода (Malware Patterns Plugin) ``malware_patterns_plugin.py``

#### Назначение
Плагин предназначен для поиска и анализа подозрительных паттернов в Mach-O файлах, которые могут указывать на наличие вредоносного кода. Плагин анализирует различные категории паттернов и оценивает уровень риска.

#### Категории анализируемых паттернов

1. **Анти-отладка (Anti-Debug)**:
...
3. **Файловые операции (File Operations)**:
   - open - открытие файла
   - write/read - работа с файлами
   - unlink - удаление файла
   - rename - переименование файла
   - chmod - изменение прав доступа
   - chown - изменение владельца
   - mkdir/rmdir - работа с директориями

4. **Управление процессами (Process)**:
   ...
   - setuid/setgid - изменение прав доступа

5. **Криптографические операции (Crypto)**:
   - crypto - криптографические операции
   - openssl - использование OpenSSL
   - aes/des/rc4 - различные алгоритмы шифрования
   - md5/sha1/sha256 - алгоритмы хеширования

6. **Shell-операции (Shell)**:
   - /bin/sh, /bin/bash - использование оболочек
   - cmd - командная строка
   - shell - общие shell-операции
   - sh -c, bash -c - выполнение shell-команд

#### Методика анализа

1. **Поиск паттернов**:
   - Анализ сегментов `__TEXT` и `__DATA`
   - Поиск паттернов с учетом регистра
   - Подсчет количества найденных паттернов
   - Категоризация найденных паттернов

2. **Оценка рисков**:
   - INFO (низкий риск) - менее 5 паттернов
   - WARNING (средний риск) - 5-10 паттернов
   - CRITICAL (высокий риск) - более 10 паттернов
...
│ Всего паттернов │ 37                     │
│ Категорий       │ 6                      │
│ Уровень риска   │ SeverityLevel.CRITICAL │
└─────────────────┴────────────────────────┘
...
┡━━━━━━━━━╇━━━━━━━━━━╇━━━━━━━━━━╇━━━━━━━━━━━━┩
│ __TEXT  │ 0x0      │ 81362944 │ anti_debug │
│ __TEXT  │ 0x0      │ 81362944 │ network    │
│ __TEXT  │ 0x0      │ 81362944 │ file_ops   │
│ __TEXT  │ 0x0      │ 81362944 │ process    │
│ __TEXT  │ 0x0      │ 81362944 │ crypto     │
│ __TEXT  │ 0x0      │ 81362944 │ shell      │
└─────────┴──────────┴──────────┴────────────┘
...
┃ Уровень                ┃ Причины                                                     ┃ Рекомендации                               ┃
┡━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┩
│ SeverityLevel.CRITICAL │ Обнаружено большое количество подозрительных паттернов (37) │ Требуется детальный анализ файла           │
│                        │ Обнаружены паттерны в нескольких категориях (6)             │ Рекомендуется запуск в изолированной среде │
│                        │ Обнаружены паттерны категории anti_debug                    │ Проверить все подозрительные категории     │
│                        │ Обнаружены паттерны категории shell                         │ Проверить паттерны категории anti_debug    │
│                        │ Обнаружены паттерны категории process                       │ Проверить паттерны категории shell         │
│                        │                                                             │ Проверить паттерны категории process       │
└────────────────────────┴─────────────────────────────────────────────────────────────┴────────────────────────────────────────────┘
```


## Примеры отчетов для различного ПО 
Отчеты с логом работы программы можно найти в директории ``./results`` 
* Steam (OS X) ([Результат](https://github.com/OlegDurandin/MachO-Analyzer/blob/main/results/steam_log.txt))
* Telegram ([Результат](https://github.com/OlegDurandin/MachO-Analyzer/blob/main/results/telegram_log.txt))
* Simple hello world application (clang) ([Результат](https://github.com/OlegDurandin/MachO-Analyzer/blob/main/results/simple_app_with_stack_canary.txt) )
* Malware sample https://bazaar.abuse.ch/sample/169ea42211969e2021784c9af170bdfe4b1fb29ae3e2c96f1bc87563f42bd9dc/  ([Результат](https://github.com/OlegDurandin/MachO-Analyzer/blob/main/results/virus_log.txt))
