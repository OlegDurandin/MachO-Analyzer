...
class SegmentAnalyzer:
    def analyze_segments(self, macho: MachO) -> Dict[str, Any]:
        """
        Анализирует сегменты и секции Mach-O файла
        """
        segments = {}
        for cmd in macho.load_commands:
            if cmd.cmd == LC_SEGMENT_64:
                segment = {
                    "name": cmd.segname,
                    "vmaddr": hex(cmd.vmaddr),
                    "vmsize": hex(cmd.vmsize),
                    "fileoff": hex(cmd.fileoff),
                    "filesize": hex(cmd.filesize),
                    "maxprot": self._get_protection(cmd.maxprot),
                    "initprot": self._get_protection(cmd.initprot),
                    "sections": self._analyze_sections(cmd)
                }
                segments[cmd.segname] = segment
        return segments

    def _analyze_sections(self, segment: SegmentCommand64) -> List[Dict[str, Any]]:
        """
        Анализирует секции в сегменте
        """
        sections = []
